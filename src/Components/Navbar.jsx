  ...
  return (
    <div className='flex w-full bg-black justify-center'>
      <div className='flex justify-evenly items-center w-[95%] sm:w-[90%] text-white p-2 sm:justify-around '>
       <h1 onClick={() => navigate("/")} className='text-[14px] sm:text-xl text-blue-500 font-sans font-semibold'>EPHEMERA</h1>
        <ul className='flex gap-6 font-semibold text-[14px] sm:text-xl'>
        ...
        {userData ? <div onClick={() => setMenuToggle(!menuToggle)}
          className='w-8 sm:w-10 sm:h-10 h-8 flex justify-center items-center rounded-full border relative bg-white text-black font-semibold text-[14px] sm:text-xl'>
          ...
          rounded pt-10`}>
            <ul className="list-none w-[20vw] sm:w-[15vw] m-1 sm:m-0 p-4 bg-gray-100 rounded-xl text-sm">
              <li onClick={() => logout()} className="text-center  text-[14px]  sm:text-[20px] sm:py-2 sm:px-2 rounded-xl hover:bg-gray-200 cursor-pointer pr-6">Logout</li>
            </ul>
          </div>
        </div> :
